$(window).scroll(function() {
    if ($(this).scrollTop()>=0 ) {//not in the end of page
      ...
      $(".Nav_item").each(function(){MinimaliseNavbar(this,45,148);});
    }
});
$( document ).ready(function() {
  ...
  $(".FadeOut").each(function(){ElementFadeOut(this); });
  $(".NavBar").each(function(){MinimaliseNavbar(this,50,150);});
  $(".Nav_item").each(function(){MinimaliseNavbar(this,48,148);});

  $(".Document").fadeTo( "slow" , 1, function() {});// fade on show
});

function ElementFadeIn(elem){//from bottom to top
  var opac=(($(window).scrollTop() + $(window).height()*(3/4)) - $(elem).offset().top) / $(elem).height();
...
$(document).ready(function(){
  $('a[href^="#"]').on('click',function (e) {
    ...
    if(target=="#top"){
      ...
      $('html, body').stop().animate({'scrollTop': $target.offset().top - 80}, 900, 'swing', function () {});//80
    }
  });
});